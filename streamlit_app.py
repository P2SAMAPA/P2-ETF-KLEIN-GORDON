...
st.set_page_config(page_title="Klein-Gordon Relativistic Quantum Field", layout="wide")
...
st.markdown('<h1 style="text-align: center;">⚛️ Klein-Gordon Relativistic Quantum Field</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Relativistic scalar field | Macro‑dependent mass | Field amplitude as momentum signal</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Klein-Gordon")
...
st.sidebar.markdown(f"**Mass base:** {config.MASS_BASE} | **Mass range:** {config.MASS_RANGE}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'klein_gordon_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Field amplitude: {etf['kg_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Amplitude"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Amplitude", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("⚛️ Top ETFs by Klein-Gordon Field Amplitude (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Klein-Gordon equation** describes relativistic scalar fields (spin‑0 particles).
        - The ETF's return series is treated as the initial field configuration.
        - The field evolves in time and space according to the wave equation with a macro‑dependent mass term.
        - The score is the **field amplitude** at the last time step – high = coherent, propagating trend/momentum.
        - Macro variables modulate the mass: higher macro → heavier mass → slower propagation.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the field amplitudes.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Klein-Gordon Field | Relativistic quantum field for momentum detection")
