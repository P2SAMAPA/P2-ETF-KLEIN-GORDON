...
from klein_gordon import compute_macro_factor, klein_gordon_field
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        s = klein_gordon_field(
            ret_window[ticker].values,
            current_macro,
            mass_base=config.MASS_BASE,
    ...
    top_etfs = [{"ticker": t, "kg_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/klein_gordon_{timestamp}.json"
